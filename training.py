...
ad_feature_importance = (
    pd.Series(ad_demo_model.feature_importances_, index=x_train.columns)
...
xg_feature_importance = (
    pd.Series(xg_demo_model.feature_importances_, index=x_train.columns)
    .sort_values(ascending=False)
    .head(10)
)
...
rf_feature_importance = (
    pd.Series(rf_demo_model.feature_importances_, index=x_train.columns)
    .sort_values(ascending=False)
    .head(10)
)
...
dt_feature_importance = (
    pd.Series(dt_demo_model.feature_importances_, index=x_train.columns)
    .sort_values(ascending=False)
    .head(10)
)
...
models = {
    "Adaboost": (
        AdaBoostRegressor(n_estimators=200, learning_rate=0.2, random_state=42),
        ad_features,
    ),
    "XGBoost": (
        XGBRegressor(n_estimators=200, learning_rate=0.2, random_state=42),
        xg_features,
    ),
...
for name, (model, features) in models.items():
    score = cross_val_score(
        model, x_train[features], y_train, cv=5
    ).mean()  # performing the cross-validation process

    if score > best_score:
        best_score, best_name, best_model, best_features = (
            score,
            name,
            model,
            features,
...
print(
    f"The best model is {best_name.upper()} with {best_score} cross-validation score."
)
...
for name, (model, features) in models.items():
    ...
    ax[row, column].plot(
        train_sizes, training_score.mean(1), color="green", label="Training Curve"
    )
    ax[row, column].plot(
        train_sizes, testing_score.mean(1), color="red", label="testing score"
    )
...
feature_importance_dictionary = {
    "Adaboost": ad_feature_importance,
    "XGBoost": xg_feature_importance,
